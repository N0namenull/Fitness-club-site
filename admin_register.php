...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        echo "Администратор с таким именем пользователя уже существует, выберите другое имя пользователя";
    } else {
        // Шифруем пароль
        ...
        $sql = "INSERT INTO admins (id, username, email, password, name)
                VALUES ('$user_id', '$username', '$email', '$hashed_password', '$fio')";
        if (mysqli_query($conn, $sql)) {
            echo '<a href="./login.html">Администратор успешно зарегестрирован, нажмите сюда чтобы войти</a>';
        } else {
